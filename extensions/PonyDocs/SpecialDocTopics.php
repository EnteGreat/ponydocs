		...
		{			
			...
			while( $row = $dbr->fetchObject( $res ))
			{
				...
				if( $versionRes->numRows( ))
				{
					...
					{	
						if( preg_match( '/^V:(.*)/i', $versionRow->cl_to, $vmatch ))
						{
							$outputData[$baseTopic]['topics'][$row->page_title][] =  $vmatch[1];
