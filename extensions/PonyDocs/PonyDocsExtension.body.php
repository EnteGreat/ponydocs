...
class PonyDocsExtension 
{
	...
	public function __construct( )
	{
		 ...
		 * PONYDOCS_DOCUMENTATION_NAMESPACE_NAME . '/<product>/<latest|version>/<manual>/<topic>'
	...
	static public function onArticleSave_CheckTOC( &$article, &$user, &$text, &$summary, $minor, $watch, $sectionanchor, &$flags )
	{
		...
		if( preg_match( '/' . PONYDOCS_DOCUMENTATION_PREFIX . '(.*):(.*)TOC(.*)/i', $title->__toString( ), $match ))
		{
...
HEREDOC;
	...
	public function onArticleFromTitleQuickLookup($title, $article) {
		...
		if (preg_match('/&action=edit/', $_SERVER['PATH_INFO'])) {
			...
			if (preg_match('/^' . str_replace('/', '\/', $wgScriptPath) . '\/' . PONYDOCS_DOCUMENTATION_NAMESPACE_NAME . '\/([' . PONYDOCS_PRODUCT_LEGALCHARS . ']+)\/(([' . PONYDOCS_PRODUCTVERSION_LEGALCHARS . ']*)\/)(\w+)\/?$/i', $_SERVER['HTTP_REFERER'], $match))
			{
